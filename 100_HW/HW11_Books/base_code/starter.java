...
class starter {
	public static void main(String args[]) throws FileNotFoundException {
		...
		// The books are: a_midsummer_nights_dream.txt, alice_in_wonderland.txt, bee_movie.txt, hamlet.txt, macbeth.txt, mobydick.txt, othello.txt, richard_III.txt, romeo_and_juliet.txt, shrek.txt, test.txt, the_merchant_of_venice.txt
		...
		*/
		while(true){
			...
			if (choice.equals("1")) {
            	...
            	System.out.println("Enter number of words to print:");
            	int num = sc.nextInt();
            	sc.nextLine(); 
            	printsNumber(arr, num);
        	} 
        	else if (choice.equals("5")) {
            	System.out.println("Please enter the file name of the new text file: (Ex: test.txt)");
	...
	*/
	public static void printsNumber(String[] arr, int num){
		if(num > arr.length){
			System.out.println("The number you entered was too large, Try again please.");
			return;
		}
		for(int i = 0; i < num; i++){
			System.out.print(arr[i] + " ");
	...
	*/
	public static String mostCommon(String[] arr){
		...
		return mostCommonWord;	// Dummy value, delete when needed.
	}
